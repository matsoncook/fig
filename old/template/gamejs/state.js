function GameState()
{
	...
	this.switchGameStateIn = function()
	{
...
function createGameObject3d(name)
{
	var go = new GameObject(3,name);
	go.position = createPoint3d(0,0,0);
	go.size =  createPoint3d(0,0,0);	
	go.velocity = createPoint3d(0,0,0);//Should be direction Vector?
	
	go.timeScaledVelocity = new Point3d();//Should be velocity units per sec?
	go.bounds = createPoint3d(0,0,0);
}
function GameObject(gameObjectType,name)
{
